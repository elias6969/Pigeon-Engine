...
bool CheckCollision(glm::vec3 &Position1, glm::vec3 &Position2, float size) {
  bool collisionX =
      Position1.x + size >= Position2.x && Position2.x + size >= Position1.x;
  bool collisionY =
      Position1.y + size >= Position2.y && Position2.y + size >= Position1.y;
  bool collisionZ =
      Position1.z + size >= Position2.z && Position2.z + size >= Position1.z;
...
void Particle::InitParticle() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(particleVertices), particleVertices,
               GL_STATIC_DRAW);

  // Define vertex attribute layout: positions (location = 0) and texture
  ...
  if (texturePath == nullptr || strlen(texturePath) == 0) {
    ...
    texturePath =
        "/home/lighht19/Documents/Pigeon-Engine/assets/Textures/masuka.jpg";
  }
  ...
  shader.LoadShaders((PathManager::shaderPath + "particle.vs").c_str(),
                     (PathManager::shaderPath + "particle.fs").c_str());
...
// RENDER PARTICLE
void Particle::renderParticles(Camera &camera, bool RenderParticle,
                               GLFWwindow *window) {
  // OPTIMIZE
  if (!RenderParticle)
    return;
  ...
  for (int i = 0; i < ParticleAmount; ++i) {
    ...
    if (particlePositions[i].y < -0.5f || particlePositions[i].y > Height) {
      particlePositions[i] =
          glm::vec3((float(rand()) / float(RAND_MAX)) * 2.0f - 1.0f,
                    (float(rand()) / float(RAND_MAX)) * 2.0f,
                    (float(rand()) / float(RAND_MAX)) * 2.0f - 1.0f);

      particleVelocities[i] =
          glm::vec3((float(rand()) / float(RAND_MAX)) * 0.05f - 0.025f,
  ...
  if (ParticleVAO == 0 || ParticleTexture == 0) {
    throw std::runtime_error("Particle system is not properly initialized. "
                             "Ensure InitParticle() is called first.");
  }
  ...
  for (const auto &position : particlePositions) {
    ...
    // model = glm::rotate(model, glm::radians(rotationAngle), glm::vec3(0.01f,
    // 0.01f, 0.01f));
...
void Grid::setupGridWater() {
  ...
  glBufferData(GL_ARRAY_BUFFER, waterVertices.size() * sizeof(float),
               waterVertices.data(), GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, waterEBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               waterIndices.size() * sizeof(unsigned int), waterIndices.data(),
               GL_STATIC_DRAW);
...
void Grid::renderGridWater(Camera &camera, GLFWwindow *window) {
  if (waterVAO == 0) {
    std::cerr
        << "Water grid VAO is not initialized! Call setupGridWater() first.\n";
    return;
  }
  ...
  glBindVertexArray(waterVAO);
  glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(waterIndices.size()),
                 GL_UNSIGNED_INT, 0);
  glBindVertexArray(0);
...
void Grid::setupGrid() {
  ...
  // Load grid shader.
  gridShader.LoadShaders((PathManager::shaderPath + "grid.vs").c_str(),
                         (PathManager::shaderPath + "grid.fs").c_str());
}
...
void Cube::render(Camera &camera, GLFWwindow *window, double &mouseX,
...
void SkyBox::texturebufferLoading(Shader &shader) {
  ...
  static bool isTextureLoaded =
      false; // Flag to check if the texture has been loaded
  if (!isTextureLoaded) {
  ...
  static bool isShaderLoaded =
      false; // Flag to check if shaders have been loaded
  if (!isShaderLoaded) {
    // Load shaders only once
    shader.LoadShaders((PathManager::shaderPath + "skybox.vs").c_str(),
                       (PathManager::shaderPath + "skybox.fs").c_str());
...
void SkyBox::renderSkybox(Shader &shader, int screenWidth, int screenHeight,
  ...
  // Draw the skybox (36 vertices: 6 faces, 2 triangles per face, 3 vertices per
  // triangle)
  glDrawArrays(GL_TRIANGLES, 0, 36);
